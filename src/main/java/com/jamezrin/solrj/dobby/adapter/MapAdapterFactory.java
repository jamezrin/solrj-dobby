 ...
 *
 * <p>This is primarily useful for Solr dynamic fields, where the field names are not known at
 * compile time and are captured in a map.
 *
  ...
  public <T> TypeAdapter<T> create(Dobby dobby, TypeToken<T> type) {
    ...
    if (TypeToken.getRawType(keyType) != String.class) {
      throw new DobbyException(
          "Dobby only supports Map<String, V> for Solr field mapping, but got key type: "
              + keyType);
    }
