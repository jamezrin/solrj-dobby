 ...
 *
 * <p>A {@code Dobby} instance is <b>immutable and thread-safe</b>. Create one via {@link
 * #builder()} and reuse it throughout your application.
 *
 * <p>Example:
 *
 * <pre>{@code
...
public final class Dobby {
  ...
  public <T> SolrInputDocument toDoc(T obj) {
    ...
    throw new DobbyException(
        "TypeAdapter for "
            + type.getName()
            + " did not produce a SolrInputDocument (got "
  ...
  /**
   * Converts a collection of Java objects to a list of {@link SolrInputDocument}s. All objects must
   * be of the same type.
   *
      ...
      if (solrObj instanceof SolrInputDocument sid) {
        ...
        throw new DobbyException(
            "TypeAdapter for "
                + type.getName()
                + " did not produce a SolrInputDocument (got "
                + (solrObj == null ? "null" : solrObj.getClass().getName())
                + ")."
                + " Only bean/record types can be converted to SolrInputDocument.");
      }
    }
  ...
  /**
   * A placeholder adapter that delegates to a real adapter once resolved. Used to break circular
